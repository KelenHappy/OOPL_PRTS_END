...
class ChosenCharacter:public Util::GameObject {
    ...
    void CreateHalfCard();
    void Settran();
    std::vector<std::shared_ptr<HalfCard>> GetHalfCard(){return m_HalfCard;}
    std::shared_ptr<TextBox> GetExit(){return m_Exit;}
    [[nodiscard]] std::vector<std::shared_ptr<Util::GameObject>> GetChildren() const {return { m_background,m_Exit};}
    std::shared_ptr<TextBox> GetExit() const {return m_Exit;}
