...
public:
    Sniper(std::string CharacterName, int IdleCont, int AttackCont,int DieCont, int StartCont) : AnimatedCharacter(){
            ...
            for(float i = 0; i < IdleCont; i += (float)IdleCont / defaultNum){
                ...
                IdleImage.emplace_back(RESOURCE_DIR"/Character/Sniper/"+CharacterName+"/Idle/"+std::to_string(i+1)+".png");
            }
            for(float i = 0; i < StartCont; i += (float)StartCont / defaultNum){
                ...
                StartImage.emplace_back(RESOURCE_DIR"/Character/Sniper/"+CharacterName+"/Start/"+std::to_string(i+1)+".png");
            }
            for(float i = 0; i < DieCont; i += (float)DieCont / defaultNum){
                ...
                DieImage.emplace_back(RESOURCE_DIR"/Character/Sniper/"+CharacterName+"/Die/"+std::to_string(i+1)+".png");
            }
            for(float i = 0; i < AttackCont; i += (float)AttackCont / defaultNum){
                ...
                AttackImage.emplace_back(RESOURCE_DIR"/Character/Sniper/"+CharacterName+"/Attack/"+std::to_string(i+1)+".png");
            }
