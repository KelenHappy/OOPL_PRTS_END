...
public:
	Medic(std::string CharacterName, int IdleCont, int AttackCont, int DieCont, int StartCont) : AnimatedCharacter() {
		...
		HalfCardImage.emplace_back(RESOURCE_DIR "/HalfCard/Medic/" + CharacterName + ".png");
		...
		auto fillImages = [&](int count, std::vector<std::string>& images, const std::string& actionFolder) {
			...
			for (int step = 0; step < defaultNum; ++step) {
				...
				seen.insert(idx);
				images.emplace_back(RESOURCE_DIR "/Character/Medic/" + CharacterName + "/" + actionFolder + "/" + std::to_string(idx + 1) + ".png");
			}
		};
