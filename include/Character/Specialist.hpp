...
public:
    Specialist(std::string CharacterName, int IdleCont, int AttackCont,int DieCont, int StartCont) : AnimatedCharacter(){
        ...
        for(float i = 0; i < IdleCont; i += (float)IdleCont / defaultNum){
            ...
            seen.insert(int_i);
            IdleImage.emplace_back(RESOURCE_DIR"/Character/Specialist/" + CharacterName + "/Idle/" + std::to_string(int_i + 1) + ".png");
        }
        ...
        for(float i = 0; i < StartCont; i += (float)StartCont / defaultNum){
            ...
            if (seen.find(int_i) != seen.end()) continue;
            seen.insert(int_i);
            StartImage.emplace_back(RESOURCE_DIR"/Character/Specialist/" + CharacterName + "/Start/" + std::to_string(int_i + 1) + ".png");
        }
        ...
        for(float i = 0; i < DieCont; i += (float)DieCont / defaultNum){
            ...
            if (seen.find(int_i) != seen.end()) continue;
            seen.insert(int_i);
            DieImage.emplace_back(RESOURCE_DIR"/Character/Specialist/" + CharacterName + "/Die/" + std::to_string(int_i + 1) + ".png");
        }
        ...
        for(float i = 0; i < AttackCont; i += (float)AttackCont / defaultNum){
            ...
            if (seen.find(int_i) != seen.end()) continue;
            seen.insert(int_i);
            AttackImage.emplace_back(RESOURCE_DIR"/Character/Specialist/" + CharacterName + "/Attack/" + std::to_string(int_i + 1) + ".png");
        }
